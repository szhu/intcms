...
def make_globals():
	...
	class pathinfo(object):
		...
		def update_parents(self):
			...
			if tail: appendtoparents(path, tail, False)
			path = base
			while True:
				base, tail = split(path)
	...
	class dirinfo():
		...
		def matches(patterns, string, flags=0):
			...
			for pattern in patterns:
				if match(pattern, string, flags): return True
			else: return False

		@property
		def is404(self):
			self.update(); return self._is404
		@property
		def is403(self):
			self.update(); return self._is403
		@property
		def filesloaded(self):
			self.update(); return self._filesloaded
		@property
		def files(self):
			self.update(); return self._files
		...
		def update(self):
			...
			try:
				...
				if exc.errno in (2, 20): self._is404 = True
				elif exc.errno is 13: self._is403 = True
				else: raise
			else:
				filesinfo = []
				fileslist.sort()
			...
			if self._is404: status.set(404)
			elif self._is403: status.set(403)
