...
def readfile_abs(path):
	...
	f.close()
	return contents
def readfile_web (*path): return readfile_abs(path_web (*path))
def readfile_page(*path): return readfile_abs(path_page(*path))

def file_web (*path): return file(path_web (*path))
def file_page(*path): return file(path_page(*path))

def exists_web (*path): from os.path import exists ; return exists(path_web (*path))
def exists_page(*path): from os.path import exists ; return exists(path_page(*path))
...
class Status(object):
	...
	def __repr__(self): return repr(self.status)
	def set(self, value): self.status = value
status = Status()
...
class Environment(JinjaEnvironment):
	...
	def create_jinja_env(page, templates_dir, http_info=None):
		...
		if http_info: env.globals.update(http_info)
		return env
	...
	def diff(path, parent):
		if path.startswith(parent): return parent, path[len(parent):]
		else: raise ValueError('%(path)r does not start with %(parent)r' % locals())
	...
	def parents(self, path):
		...
		if not tail: head, tail = split(head)
		while True:
			if not (head and tail): break
			yield self.diff(path, head)
			head, tail = split(head)
	...
	def get_page_redirect(env, page):
		...
		for filename in REDIRECT_PAGES:
			if not exists_web(page, filename): continue
		...
		for filename in REDIRECT_WILDCARD_PAGES:
			if not exists_web(parent, filename): continue
	...
	def get_page_regular(env, page):
		...
		try:
			...
			if exc.name not in PAGE_TEMPLATES: raise
	...
	def get_page(env, page):
		return (
			env.get_page_redirect(page) or
			env.get_page_error   (page) or
			env.get_page_regular (page)
		)
...
class Response(object):
	...
	def print_header(self):
		if not self.header_parts: raise self.ResponseException('header is empty')
		print self.header ; print
		import sys ; sys._finishedprintingheader = True
	def print_content(self):
		if self.is_html: print self.content_with_bom.encode('utf-8')
